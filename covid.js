...
const renderCovid = (cData) => {
    ...
    // dataEl.appendChild (lineEl)
    console.log(totalNew)
    document.querySelector('#total-cases').textContent = totalCases
...
getCovid(url).then((covidData) => {
    ...
    stateName = covidData[3].state
    console.log(stateName)

    document.querySelector('#state-name').textContent = stateName

    for (var item in covidData[3].districtData) {
        ...
        keralaData.push({                                           //get required information to the local array
            district: covidData[3].districtData[item].district,     //get district name
            confirmed: covidData[3].districtData[item].confirmed,   //get total confirmed cases
            new: covidData[3].districtData[item].delta.confirmed    //get the new cases today figures
        })
    }
